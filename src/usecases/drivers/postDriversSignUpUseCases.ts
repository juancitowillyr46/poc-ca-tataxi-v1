...
export class postDriversSignUpUseCases {
  constructor(
  ...
  async execute(signUp: DriverSignUpM): Promise<boolean> {

    let userM = new UserM();
    let success = true;
    let isEqualPws = await signUp.validateConfirmPassword(signUp.password, signUp.confirmPassword);
